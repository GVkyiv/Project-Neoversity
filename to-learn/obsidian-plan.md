...
Магистратура Neoversity это обязательная база, она идёт первой всегда.
Этот план по Obsidian и всё остальное из [backlog.md](backlog.md) это
добавка поверх, а не замена. Правило на неделю: если по курсу идёт тема
с оцениваемым ДЗ, финальное тестирование или защита проекта, модуль
Obsidian на этой неделе пропускается и сдвигается, а не делается
параллельно в ущерб курсу.

С 04.09.2026 параллельно с этим планом идёт
[OpenRouter](openrouter-plan.md) и курсы Anthropic Academy из
[backlog.md](backlog.md), поэтому правило приоритета работает сразу по
всем трекам: сдвигается вся добавка целиком, а не один трек из трёх.

Текущая дисциплина, её даты и статус: [CALENDAR.md](../CALENDAR.md).
Что уже сдано и на какой теме курс: [STATE.md](../STATE.md). Список тем
и веса ДЗ по каждой дисциплине: [PROGRAM.md](../PROGRAM.md). Эти три
файла источник правды по учёбе, здесь ничего из них не дублируется.

Понедельничное письмо ставит блок по Neoversity первым, до блока про
Obsidian, и собирает его из этих трёх файлов.
...
Поле `queue` во frontmatter задаёт порядок **внутри трека**: 1-4 здесь и
отдельно 1-4 в [OpenRouter](openrouter-plan.md). С 04.09.2026 треки идут
параллельно, а не один за другим, и письмо-план выдаёт по одному модулю
из каждого активного трека: первый, у которого `status` не равен `done`.
Даты недели лежат в поле `week` во frontmatter модуля.
...
Письма генерируются облачной рутиной из GitHub. Полная инструкция для
рутины лежит в [prompts/weekly-study-email.md](../prompts/weekly-study-email.md),
там же разобрано, что было не так со старой версией писем.

**Понедельник 10:00.** Что учим на этой неделе: по одному пункту с
каждого активного трека (Obsidian, OpenRouter, Anthropic Academy), с
короткой выжимкой и ссылкой `obsidian://` на локальный файл. Общий список
тем в это письмо не входит.

**Пятница 18:00.** Напоминание обновить статусы. По этому письму нужно
проставить, что сделано, а что нет.

**1 числа месяца, 10:00.** Полный обзор всех тем из
[backlog.md](backlog.md), включая отложенные. Отдельно от недельного
письма, чтобы недельное оставалось коротким.
...
| Spaced Repetition | ставим | модуль 3 | Флеш-карты по темам на "выучить" (OpenRouter, Anthropic Academy), карточки Вопрос::Ответ пишет Claude Code, плагин только показывает их по алгоритму, без LLM внутри |
...
**Сбор плагинов.** Из 2700+ community-плагинов в плане семь, и ставятся
